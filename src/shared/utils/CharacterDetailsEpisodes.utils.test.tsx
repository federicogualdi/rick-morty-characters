...
import { episodesCharacter } from './CharacterDetailsEpisodes.utils';
...
test('verify correct function of episodes name slicing', async () => {
  const slicedEpisodes = episodesCharacter(episodes, 5);

  expect(slicedEpisodes?.split(',').length).toBe(5);
  expect(slicedEpisodes?.split(',').length).toBeLessThan(episodes.map((e) => e.name).length);
});
